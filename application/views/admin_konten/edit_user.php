              ...
              <form id="formedit" name="datauser" class="form-horizontal" role="form" action="<?php echo base_url();?>operation/change_useremail" method="post" >
                ...
                <div class="form-group">
                  <label class="col-sm-1 control-label" for="lg">Role</label>
                    <div class="col-sm-5">
                      <select name='role' id='role'  class="form-control" <?php if (($session['role'] != 'Admin') OR ($data_user['id']==1)) echo 'disabled';?>>
                        <?php //if (isset($data_user)){}else{echo '<option value=0>-- Pilih Role --</option>'; }?>
                        <?php 
                            foreach($hak as $hak)
                            {   ?>      
                                <option value="<?php echo $hak['id']; ?>" <?php if($hak['nama']==$data_user['role']){echo 'selected="selected"';}else{}?>><?php echo $hak['nama']; ?></option>
                         <?php   }
                        ?>
                      </select>
                    </div>
